...
class StoryNamespace(Namespace):
    ...
    def handle_current_name_as_package(self, *, name, path, resolution, package_file_content, sub_directories, definitions):
        ...
        if isinstance(story_dict, dict):
            try:
                story = UserStory(**Structure.Story(**story_dict).as_dict())
            except Exception as e:
                import traceback
                raise DefinitionIsInvalidError(self, name=name, path=path, resolution=resolution, error=f"Allowed dictionary keys are [{Structure.Story.keys}]. Overall structure definition is {str(Structure.Story.definition)}. Error: {e}. Check: {traceback.format_exc()} ")
