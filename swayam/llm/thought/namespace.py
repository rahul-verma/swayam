...
class ThoughtNamespace(Namespace):
    ...
    def handle_current_name_as_package(self, *, name, path, resolution, package_file_content, sub_directories, definitions):
        ...
        if isinstance(thought_dict, dict):
            try:
                thought = UserThought(**Structure.Thought(**thought_dict).as_dict())
            except Exception as e:
                import traceback
                raise DefinitionIsInvalidError(self, name=name, path=path, resolution=resolution, error=f"Allowed dictionary keys are [{Structure.Thought.keys}]. Overall structure definition is {str(Structure.Thought.definition)}. Error: {e}. Check: {traceback.format_exc()} ")
